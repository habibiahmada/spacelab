...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterContent() {
        ...
        buildingCards.forEach(card => {
            ...
            if (searchTerm) {
                buildingMatches = buildingName.includes(searchTerm) || buildingCode.includes(searchTerm);
            } else {
    ...
    async function editBuilding(buildingId) {
        try {
            ...
            document.getElementById('editBuildingFloors').value = building.total_floors;
            document.getElementById('editBuildingDescription').value = building.description || '';

            // Set form action
            document.getElementById('editBuildingForm').action = `/staff/buildings/${building.id}`;

            // Open modal
            window.dispatchEvent(new CustomEvent('open-modal', { detail: 'edit-building-modal' }));
        } catch (error) {
    ...
    function deleteBuilding(buildingId, buildingName) {
        ...
        document.getElementById('deleteBuildingForm').action = `/staff/buildings/${buildingId}`;

        window.dispatchEvent(new CustomEvent('open-modal', { detail: 'delete-building-modal' }));
    }
    ...
    async function editRoom(roomId) {
        try {
            ...
            // Set form action
            document.getElementById('editRoomForm').action = `/staff/rooms/${room.id}`;

            // Open modal
            window.dispatchEvent(new CustomEvent('open-modal', { detail: 'edit-room-modal' }));
        } catch (error) {
    ...
    function deleteRoom(roomId, roomName) {
        ...
        window.dispatchEvent(new CustomEvent('open-modal', { detail: 'delete-room-modal' }));
    }
});
